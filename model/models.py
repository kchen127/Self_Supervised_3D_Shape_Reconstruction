...
class Encoder(nn.Module):
    def __init__(self, dim_in=4, dim_out=512, dim1=64, dim2=1024, im_size=64):
        ...
        # self.encoder_layer = nn.TransformerEncoderLayer(d_model=dim_hidden[2]*math.ceil(im_size/8)**2, nhead=8)
        # self.encoder_layer = nn.TransformerEncoderLayer(d_model=dim_hidden[3], nhead=8)
        # self.encoder_layer = nn.TransformerEncoderLayer(d_model=dim_hidden[4], nhead=8)
...
class Model(nn.Module):
    ...
    def loss_iou(self, image_a, image_b, viewpoint_a, viewpoint_b, voxels):
        # [Ia, Ib]
        images = torch.cat((image_a, image_b), dim=0)
        # [Va, Va, Vb, Vb], set viewpoints
        viewpoints = torch.cat((viewpoint_a, viewpoint_a, viewpoint_b, viewpoint_b), dim=0)
        ...
        vertices_a, faces_a = self.reconstruct(image_a)
        faces_ = srf.face_vertices(vertices_a, faces_a).data
        faces_norm = faces_ * 1. * (32. - 1) / 32. + 0.5
        voxels_predict = srf.voxelization(faces_norm, 32, False).cpu().numpy()
        voxels_predict = voxels_predict.transpose(0, 2, 1, 3)[:, :, :, ::-1]
        iou = (voxels * voxels_predict).sum((1, 2, 3)) / (0 < (voxels + voxels_predict)).sum((1, 2, 3))
        
        return silhouettes.chunk(4, dim=0), laplacian_loss, flatten_loss, iou
    
    def forward(self, images=None, viewpoints=None, voxels=None,task="train"):
        if task=="train":
            return self.loss_iou(images[0], images[1], viewpoints[0], viewpoints[1], voxels)
        elif task=="test":
